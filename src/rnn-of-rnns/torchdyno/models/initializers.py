...
def lower_triangular(
    ...
    Returns:
        torch.Tensor: initialized lower triangular tensor.
    """
    if len(shape) != 2:
        raise ValueError("Lower triangular matrix must be 2D.")
    if shape[0] != shape[1]:
        raise ValueError("Lower triangular matrix must be square.")

    return torch.tril(torch.empty(shape, dtype=dtype))
