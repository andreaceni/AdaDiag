...
# Shared collate function for HAR2, FordA, FloodModeling1, etc.
...
# TODO: Add other datasets here as needed
# FloodModeling
# for modularity experiments: JapaneseVowels, (BasicMotions), sCIFAR10.
# FordB, Adiac, Eigenworms, etc. 


# FloodModeling1 utilities 
def load_FloodModeling1_dataloaders(train_batch_size: int = 64, test_batch_size: int = 64) -> Tuple[torch.utils.data.DataLoader, torch.utils.data.DataLoader, torch.utils.data.DataLoader]:
    """
    Load FloodModeling1 dataset from aeon and return train/val/test DataLoaders.
    Returns:
        Tuple of (train_loader, val_loader, test_loader)
    """
    ...
    # aeon returns a numpy array of shape (N_samples, inp_size, seq_len) = (673, 1, 266)
    X = to_numpy3d(X)
    y = np.array(y).astype(np.float32)
    print("FloodModeling1 shapes:", X.shape, y.shape)  # (673, 1, 266), (673,)
    ...
    # need to convert to (673, 266, 1) for torch dataloaders
...
def get_FloodModeling1_input_output_sizes():
    # FloodModeling1 is univariate, regression
    return 1, 1



# FloodModeling2 utilities 
def load_FloodModeling2_dataloaders(train_batch_size: int = 64, test_batch_size: int = 64) -> Tuple[torch.utils.data.DataLoader, torch.utils.data.DataLoader, torch.utils.data.DataLoader]:
    """
    Load FloodModeling2 dataset from aeon and return train/val/test DataLoaders.
    Returns:
        Tuple of (train_loader, val_loader, test_loader)
    """
    from aeon.datasets import load_regression
    ...
    print("FloodModeling2 shapes:", X.shape, y.shape)  # 
    X = torch.tensor(X, dtype=torch.float32)  
    y = torch.from_numpy(y).float()  

    # Filter out empty sequences if any
    non_empty_mask = np.array([len(seq) > 0 for seq in X])
    X = X[non_empty_mask]
    y = y[non_empty_mask]
...
def get_FloodModeling2_input_output_sizes():
    # FloodModeling2 is univariate, regression
    return 1, 1


# FloodModeling3 utilities 
def load_FloodModeling3_dataloaders(train_batch_size: int = 64, test_batch_size: int = 64) -> Tuple[torch.utils.data.DataLoader, torch.utils.data.DataLoader, torch.utils.data.DataLoader]:
    """
    Load FloodModeling3 dataset from aeon and return train/val/test DataLoaders.
    Returns:
        Tuple of (train_loader, val_loader, test_loader)
    """
    from aeon.datasets import load_regression
    ...
    #X = to_numpy3d(X)
    X = to_numpy3d_ieeeppg(X) # this dataset I think has variable-length sequences so it needs to be handled properly
    y = np.array(y).astype(np.float32)
    # aeon returns a numpy array of shape (N_samples, inp_size, seq_len) 
    print("FloodModeling3 shapes:", X.shape, y.shape)  # 
    X = torch.tensor(X, dtype=torch.float32)  
    y = torch.from_numpy(y).float()  
...
def get_FloodModeling3_input_output_sizes():
    # FloodModeling3 is univariate, regression
    return 1, 1




# LiveFuelMoistureContent utilities 
def load_LiveFuelMoistureContent_dataloaders(train_batch_size: int = 64, test_batch_size: int = 64) -> Tuple[torch.utils.data.DataLoader, torch.utils.data.DataLoader, torch.utils.data.DataLoader]:
    """
    Load LiveFuelMoistureContent dataset from aeon and return train/val/test DataLoaders.
    Returns:
        Tuple of (train_loader, val_loader, test_loader)
    """
    from aeon.datasets import load_regression
    ...
    X = to_numpy3d(X)
    y = np.array(y).astype(np.float32)
    print("LiveFuelMoistureContent shapes:", X.shape, y.shape)  # 
    X = torch.tensor(X, dtype=torch.float32)  
    y = torch.from_numpy(y).float()  
...
def get_LiveFuelMoistureContent_input_output_sizes():
    # LiveFuelMoistureContent is multivariate, regression
    return 7, 1



# AustraliaRainfall utilities
def load_AustraliaRainfall_dataloaders(train_batch_size: int = 64, test_batch_size: int = 64) -> Tuple[torch.utils.data.DataLoader, torch.utils.data.DataLoader, torch.utils.data.DataLoader]:
    """
    Load AustraliaRainfall dataset from aeon and return train/val/test DataLoaders.
    Returns:
        Tuple of (train_loader, val_loader, test_loader)
    """
    from aeon.datasets import load_regression
    ...
    X = to_numpy3d(X)
    y = np.array(y).astype(np.float32)
    print("AustraliaRainfall shapes:", X.shape, y.shape)  # 
    X = torch.tensor(X, dtype=torch.float32)  
    y = torch.from_numpy(y).float()  
...
def get_AustraliaRainfall_input_output_sizes():
    # AustraliaRainfall is multivariate, regression
    return 3, 1
...
# BIDMCHR utilities
def load_BIDMCHR_dataloaders(train_batch_size: int = 64, test_batch_size: int = 64) -> Tuple[torch.utils.data.DataLoader, torch.utils.data.DataLoader, torch.utils.data.DataLoader]:
    """
    Load BIDMCHR dataset from aeon and return train/val/test DataLoaders.
    Returns:
        Tuple of (train_loader, val_loader, test_loader)
    """
    from aeon.datasets import load_regression
    ...
    X = to_numpy3d_ieeeppg(X)
    y = np.array(y).astype(np.float32)
    print("BIDMCHR shapes:", X.shape, y.shape)  # 
    X = torch.tensor(X, dtype=torch.float32)  
    y = torch.from_numpy(y).float()  
...
def get_BIDMCHR_input_output_sizes():
    # BIDMCHR is multivariate, regression
    return 2, 1


# BIDMCSpO2 utilities
def load_BIDMCSpO2_dataloaders(train_batch_size: int = 64, test_batch_size: int = 64) -> Tuple[torch.utils.data.DataLoader, torch.utils.data.DataLoader, torch.utils.data.DataLoader]:
    """
    Load BIDMCSpO2 dataset from aeon and return train/val/test DataLoaders.
    Returns:
        Tuple of (train_loader, val_loader, test_loader)
    """
    from aeon.datasets import load_regression
    ...
    X = to_numpy3d_ieeeppg(X)
    y = np.array(y).astype(np.float32)
    print("BIDMCSpO2 shapes:", X.shape, y.shape)  # 
    X = torch.tensor(X, dtype=torch.float32)  
    y = torch.from_numpy(y).float()  
...
def get_BIDMCSpO2_input_output_sizes():
    # BIDMCSpO2 is multivariate, regression
    return 2, 1


# HouseholdPowerConsumption1 utilities
def load_HouseholdPowerConsumption1_dataloaders(train_batch_size: int = 64, test_batch_size: int = 64) -> Tuple[torch.utils.data.DataLoader, torch.utils.data.DataLoader, torch.utils.data.DataLoader]:
    """
    Load HouseholdPowerConsumption1 dataset from aeon and return train/val/test DataLoaders.
    Returns:
        Tuple of (train_loader, val_loader, test_loader)
    """
    from aeon.datasets import load_regression
    ...
    X = to_numpy3d_ieeeppg(X)
    y = np.array(y).astype(np.float32)
    print("HouseholdPowerConsumption1 shapes:", X.shape, y.shape)  # 
    X = torch.tensor(X, dtype=torch.float32)  
    y = torch.from_numpy(y).float()  
...
def get_HouseholdPowerConsumption1_input_output_sizes():
    # HouseholdPowerConsumption1 is multivariate, regression
    return 5, 1


# NewsHeadlineSentiment utilities
def load_NewsHeadlineSentiment_dataloaders(train_batch_size: int = 64, test_batch_size: int = 64) -> Tuple[torch.utils.data.DataLoader, torch.utils.data.DataLoader, torch.utils.data.DataLoader]:
    """
    Load NewsHeadlineSentiment dataset from aeon and return train/val/test DataLoaders.
    Returns:
        Tuple of (train_loader, val_loader, test_loader)
    """
    from aeon.datasets import load_regression
    ...
    X = to_numpy3d_ieeeppg(X)
    y = np.array(y).astype(np.float32)
    print("NewsHeadlineSentiment shapes:", X.shape, y.shape)  # 
    X = torch.tensor(X, dtype=torch.float32)  
    y = torch.from_numpy(y).float()  
...
def get_NewsHeadlineSentiment_input_output_sizes():
    # NewsHeadlineSentiment is multivariate, regression
    return 3, 1
...
# AppliancesEnergy utilities
def load_AppliancesEnergy_dataloaders(train_batch_size: int = 64, test_batch_size: int = 64) -> Tuple[torch.utils.data.DataLoader, torch.utils.data.DataLoader, torch.utils.data.DataLoader]:
    """
    Load AppliancesEnergy dataset from aeon and return train/val/test DataLoaders.
    Returns:
        Tuple of (train_loader, val_loader, test_loader)
    """
    from aeon.datasets import load_regression
    ...
    try:
        ...
        X = to_numpy3d_ieeeppg(X)
        y = np.array(y).astype(np.float32)
        print("AppliancesEnergy shapes:", X.shape, y.shape)  # 
        X = torch.tensor(X, dtype=torch.float32)  
        y = torch.from_numpy(y).float()  
        ...
        print(f"Skipping AppliancesEnergy due to load error: {e}")
        return None, None, None

def get_AppliancesEnergy_input_output_sizes():
    # AppliancesEnergy is multivariate, regression
    return 24, 1
...
# Blink utilities
def load_Blink_dataloaders(train_batch_size: int = 64, test_batch_size: int = 64) -> Tuple[torch.utils.data.DataLoader, torch.utils.data.DataLoader, torch.utils.data.DataLoader]:
    """
    Load Blink dataset from aeon and return train/val/test DataLoaders.
    Returns:
        Tuple of (train_loader, val_loader, test_loader)
    """
    from aeon.datasets import load_classification
    X_train, y_train = load_classification("Blink", split="train", load_equal_length=True, load_no_missing=False)
    X_test, y_test = load_classification("Blink", split="test", load_equal_length=True, load_no_missing=False)

    # blink has string labels ['shortblink', 'longblink'], need to convert to integers
    # Map string labels to integers
    label_mapping = {'shortblink': 0, 'longblink': 1}
...
def get_Blink_input_output_sizes():
    # Blink is multivariate, multi-class classification
    return 4, 2
...
###############################################################################################
# Utilities for Heartbeat, FaceDetection, MotionSenseHAR, EMOPain, UWaveGestureLibrary
###############################################################################################
...
# FaceDetection
def load_FaceDetection_dataloaders(train_batch_size=64, test_batch_size=64):
    return load_generic_dataloaders("FaceDetection", train_batch_size, test_batch_size)

def get_FaceDetection_input_output_sizes():
    return 144, 2   # 144 channels, binary classification


# MotionSenseHAR
def load_MotionSenseHAR_dataloaders(train_batch_size=64, test_batch_size=64):
    return load_generic_dataloaders("MotionSenseHAR", train_batch_size, test_batch_size)

def get_MotionSenseHAR_input_output_sizes():
    return 12, 6   # 12 sensor channels, 6 activity classes


# EMOPain
def load_EMOPain_dataloaders(train_batch_size=64, test_batch_size=64):
    return load_generic_dataloaders("EMOPain", train_batch_size, test_batch_size)

def get_EMOPain_input_output_sizes():
    return 30, 3   
